...
def get_by_id(
    ...
    """
        ...
        salt vm_minion vmc_public_ip.get_by_id hostname=nsxt-manager.local public_ip_id ...
    ...
    api_url = api_url.format(base_url=api_url_base, org_id=org_id,
                             sddc_id=sddc_id, public_ip_id=public_ip_id)

    return vmc_request.call_api(method=vmc_constants.GET_REQUEST_METHOD,
                                url=api_url,
                                refresh_key=refresh_key,
                                authorization_host=authorization_host,
                                description="vmc_public_ip.get_by_id",
                                responsebody_applicable=True,
                                verify_ssl=verify_ssl,
                                cert=cert)
...
def create(
    ...
    api_url = '{base_url}vmc/reverse-proxy/api/orgs/{org_id}/sddcs/{sddc_id}/' \
              'cloud-service/api/v1/infra/public-ips/{public_ip_id}'
    api_url = api_url.format(base_url=api_url_base, org_id=org_id,
                             sddc_id=sddc_id, public_ip_id=public_ip_name)
