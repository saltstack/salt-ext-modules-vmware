...
def _create_payload_for_security_rule(rule_id, domain_id, user_input):
    ...
    if domain_id == "mgw":
        rel_path = "templates/create_security_rules_mgw.json"
    else:
        rel_path = "templates/create_security_rules_cgw.json"
...
def _update_payload_for_security_rule(existing_data, user_input):
    """
    This function creates the payload for update security rule based on the json template, existing data
    and user input passed
    """
    script_dir = os.path.dirname(__file__)  # <-- absolute dir the script is in
    rel_path = "templates/update_security_rules.json"
...
def get(
    ...
    api_url = api_url.format(base_url=api_url_base, org_id=org_id, sddc_id=sddc_id, domain_id=domain_id)
...
def update(
    ...
    payload = _update_payload_for_security_rule(existing_data, req_data)
