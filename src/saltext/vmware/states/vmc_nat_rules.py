...
try:
    from saltext.vmware.modules import vmc_nat_rules
...
def __virtual__():
    if not HAS_NAT_RULES:
        return False, "'vmc_nat_rules' binary not found on system"
...
def present(
    ...
    if __opts__.get("test"):
        ...
        if existing_nat_rule:
            return vmc_state._create_state_response(
                name=name,
                comment="State present will update Nat rule {}".format(nat_rule))
        else:
            return vmc_state._create_state_response(
                name=name,
                comment="State present will create Nat rule {}".format(nat_rule))
    ...
    if existing_nat_rule:
        ...
        if is_update_required:
            ...
            if "error" in updated_nat_rule:
                return vmc_state._create_state_response(
                    name=name,
                    comment=updated_nat_rule["error"],
                    result=False
                )
            ...
            if "error" in get_nat_rule_after_update:
                return vmc_state._create_state_response(
                    name=name,
                    comment=get_nat_rule_after_update["error"],
                    result=False
                )
            ...
            return vmc_state._create_state_response(
                name=name,
                comment="Nat rule exists already, no action to perform",
                result=True
            )
    else:
        ...
        if "error" in created_nat_rule:
            return vmc_state._create_state_response(
                name=name,
                comment=created_nat_rule["error"],
                result=False
            )
...
def absent(
    ...
    if __opts__.get("test"):
        ...
        if existing_nat_rule:
            return vmc_state._create_state_response(
                name=name,
                comment="State absent will delete Nat rule with Id {}".format(nat_rule))
        else:
            return vmc_state._create_state_response(
                name=name,
                comment="State absent will do nothing as no Nat rule found with Id {}".format(nat_rule))
    ...
    if existing_nat_rule:
        ...
        if "error" in deleted_nat_rule:
            return vmc_state._create_state_response(
                name=name,
                comment=deleted_nat_rule["error"],
                result=False
            )
        ...
        return vmc_state._create_state_response(
            name=name,
            comment="No Nat rule found with Id {}".format(nat_rule),
            result=True
        )
