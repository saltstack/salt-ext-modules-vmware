...
def maintenance_mode(name,
    ...
    host_state = __salt__["vmware_esxi.in_maintenance_mode"](host=name, service_instance=service_instance)
    ...
    if __opts__["test"]:
        ret["result"] = None
        ret["changes"] = {"new": f"Host will enter {'Maintenance' if enter_maintenance_mode else 'Normal'} mode."}
        ret["comment"] = "These options are set to change."
        return ret

    if enter_maintenance_mode:
        host_state = __salt__["vmware_esxi.maintenance_mode"](host=name,
                                                              timeout=0,
        ...
        host_state = __salt__["vmware_esxi.exit_maintenance_mode"](host=name,
                                                                   timeout=0,
                                                                   catch_task_error=True,
                                                                   service_instance=service_instance)
    ...
    if ret["result"]:
        ret["changes"] = {"new": f"Host entered {'Maintenance' if enter_maintenance_mode else 'Normal'} mode."}
    else:
        ret["comment"] = f"Failed to put host {str(name)} in {'Maintenance' if enter_maintenance_mode else 'Normal'} mode."
    return ret
