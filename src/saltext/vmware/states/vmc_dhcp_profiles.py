...
try:
    ...
    HAS_DHCP_PROFILES = True
except ImportError:
    HAS_DHCP_PROFILES = False


def __virtual__():
    if not HAS_DHCP_PROFILES:
        return False, "'vmc_dhcp_profiles' binary not found on system"
...
def present(
    ...
    if "error" not in get_dhcp_profile:
        log.info("DHCP Profile found with Id %s", dhcp_profile_id)
        existing_dhcp_profile = get_dhcp_profile
    elif DHCP_PROFILE_NOT_FOUND_ERROR not in get_dhcp_profile["error"]:
        return vmc_state._create_state_response(
            name=name, comment=get_dhcp_profile["error"], result=False
        )
    ...
    if existing_dhcp_profile:
        ...
        is_update_required = vmc_state._check_for_updates(
            existing_dhcp_profile, input_dict, updatable_keys, ["tags"]
        )
        ...
        if is_update_required:
            ...
            if "error" in get_dhcp_profile_after_update:
                return vmc_state._create_state_response(
                    name=name, comment=get_dhcp_profile_after_update["error"], result=False
                )

            return vmc_state._create_state_response(
                name=name,
                comment="Updated DHCP Profile {}".format(dhcp_profile_id),
                old_state=existing_dhcp_profile,
                new_state=get_dhcp_profile_after_update,
                result=True,
            )
        else:
