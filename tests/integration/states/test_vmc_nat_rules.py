...
def nat_rule_test_data():
    ...
    return tier1, nat, nat_rule, translated_network, source_network, updated_display_name, updated_tags
...
def delete_nat_rule(vmc_nsx_connect, nat_rule_test_data):
    ...
    tier1, nat, nat_rule, translated_network, source_network, updated_display_name, updated_tags = nat_rule_test_data

    url = 'https://{hostname}/vmc/reverse-proxy/api/orgs/{org_id}/sddcs/{sddc_id}/' \
    ...
    if nat_rules_dict['result_count'] != 0:
        ...
        for result in results:
            if result['id'] == nat_rule:
                ...
                api_url = url.format(hostname=hostname, org_id=org_id, sddc_id=sddc_id, tier1=tier1,
                                     nat=nat, nat_rule=nat_rule)
...
def test_vmc_nat_rules_state_module(salt_call_cli, delete_nat_rule, vmc_nsx_connect, nat_rule_test_data):
