...
def test_manage_create(patch_salt_globals_tag_state, patch_salt_globals_tag):
    ...
    if len(cats['categories']) > 0:
        res = tagging_state.manage('create', tag_name='state tag', tag_description='testing state tag create', category_id=cats['categories'][0])
    ...
    if len(tags['tags']) > 0:
        for tag in tags['tags']:
            ...
            if res["tag"]["name"] == 'state tag':
                update_res = tagging_state.manage('update', tag_description='new discription', tag_id=res["tag"]["id"])
