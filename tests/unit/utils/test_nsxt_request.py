...
def test_success_response_with_ssl_verification_disabled(mock_request):
    ...
    assert nsxt_request.call_api(method="get",
                                 url="mock_url",
                                 username="username",
                                 password="password",
...
def test_success_response_for_delete_api_call(mock_request):
    ...
    assert nsxt_request.call_api(method="delete",
                                 url="mock_url",
                                 username="username",
                                 password="password",
                                 verify_ssl=False) is None
...
def test_for_auth_error(mock_request):
    ...
    response = nsxt_request.call_api(method="get",
                                     url="mock_url",
                                     username="username",
                                     password="password",
                                     verify_ssl=False)
...
def test_with_ssl_verification(mock_request):
    ...
    assert nsxt_request.call_api(method="get",
                                 ...
                                 cert_common_name="sample.nsxt-hostname.vmware") == _mock_success_response
...
def test_update_with_ssl_error(mock_request):
    ...
    assert response["error"] == "SSL Error occurred while calling NSX-T API mock_url." \
...
def test_update_with_request_error(mock_request):
    ...
    assert response["error"] == "Error occurred while calling NSX-T API mock_url. Please check logs for more " \
                                "details."
