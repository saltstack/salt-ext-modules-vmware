...
def mock_pyvmomi_storage_policy_object(name):
    return mock_with_name(
        ...
        constraints = Mock(spec=pbm.profile.CapabilityConstraints,
            dynamicProperty = [],
            subProfiles = [
                mock_with_name(
                    dynamicProperty = [],
...
def mocked_storage_policies_data(request, fake_service_instance):
    with patch("pyVmomi.pbm.ServiceInstance", autospec=True) as fake_pbmSi:
        ...
        fake_pbmSi.return_value.RetrieveContent.return_value.profileManager.PbmQueryProfile.return_value = vmomi_content.keys()
        fake_pbmSi.return_value.RetrieveContent.return_value.profileManager.PbmRetrieveContent.return_value = vmomi_content.values()
        for policy_name in vmomi_content.keys():
            yield policy_name, request.param["policies"], request.param["updates"], request.param["expected_changes"]


@pytest.mark.parametrize('test_run', [True, False])
def test_drift_report_storage_policies(mocked_storage_policies_data, fake_service_instance, test_run):
    ...
    with patch.dict(storage_policies.__salt__, {"storage_policies.find": list_result}):
        with patch.dict(storage_policies.__opts__, {"test": test_run}):
            ...
            if not test_run:
                policy_name = "Old Test VM Storage Policy" if config_name == 'Test case 3' else "Test VM Storage Policy"
