...
_mocked_auth_error = {"error": "The credentials were incorrect or the account specified has been locked."}
_mocked_error_response = {'module_name': 'common-services',
...
def test_publish_fqdns_enabled_without_parameters():
    with pytest.raises(TypeError) as exc:
        ...
        assert str(exc.value) == "publish_fqdns_enabled() missing 4 required positional arguments: 'name', " \
...
def test_publish_fqdns_disabled_without_parameters():
    ...
    assert str(exc.value) == "publish_fqdns_disabled() missing 4 required positional arguments: 'name', " \
                             "'hostname', 'username', and 'password'"
...
def test_publish_fqdns_enabled_with_execution_error():
    ...
    with patch.dict(
        ...
        result = nsxt_manager.publish_fqdns_enabled(name="test_publish_using_basic_auth",
    ...
    assert "The credentials were incorrect or the account specified has been locked." == result["comment"]
...
def test_publish_fqdns_disabled_when_enabled_with_basic_auth():
    ...
    with patch.dict(
        ...
        result = nsxt_manager.publish_fqdns_disabled(name="test_publish_using_basic_auth",
                                                     hostname=_mocked_hostname,
                                                     username="username",
                                                     password="password")
...
def test_publish_fqdns_disabled_test_mode():
    ...
    assert result
    assert {} == result["changes"]
    assert "State publish_fqdns_disabled will execute with params test_publish_using_basic_auth, nsx-t.vmware.com, " \
...
def test_publish_fqdns_enabled_test_mode():
    ...
    assert result
    assert {} == result["changes"]
    assert "State publish_fqdns_enabled will execute with params test_publish_using_basic_auth, nsx-t.vmware.com, " \
           "username, password, True" == result["comment"]
    assert result["result"] is None
