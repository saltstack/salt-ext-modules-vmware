...
def test_present_with_error_from_get_by_display_name():
    ...
    with patch.dict(
        ...
        result = nsxt_ip_blocks.present(name="test_present_using_basic_auth",
...
def test_present_to_create_in_test_mode():
    ...
    with patch.dict(
        ...
        with patch.dict(nsxt_ip_blocks.__opts__, {"test": True}):
            result = nsxt_ip_blocks.present(name="test_absent_using_basic_auth",
                                            hostname=mocked_hostname,
                                            username="username",
                                            cidr="1.1.1.1/24",
                                            password="password",
    ...
    assert result is not None
    assert result["changes"].__len__() == 0
    assert result["comment"] == "State present will create IP Block with name {}".format(display_name)
...
def test_present_to_update_in_test_mode():
    ...
    assert result is not None
    assert result["changes"].__len__() == 0
    assert result["comment"] == "State present will update IP Block with name {}".format(display_name)
    assert result["result"] is None
...
def test_present_when_multiple_blocks_with_same_display_name():
    ...
    assert result is not None
    assert result["changes"].__len__() == 0
    assert result["comment"] == "Multiple IP Blocks found for the provided display name {}".format(display_name)
...
def test_absent_to_delete_with_basic_auth():
    ...
    with patch.dict(
        ...
        result = nsxt_ip_blocks.absent(name="test_absent_using_basic_auth",
                                       hostname=mocked_hostname,
                                       username="username",
                                       password="password",
                                       display_name=display_name)
...
def test_absent_do_nothing_with_basic_auth():
    ...
    with patch.dict(
        ...
        result = nsxt_ip_blocks.absent(name="test_publish_using_basic_auth",
                                       hostname=mocked_hostname,
                                       username="username",
                                       password="password",
                                       display_name=display_name)
...
def test_absent_to_delete_in_test_mode():
    ...
    assert result is not None
    assert result["changes"].__len__() == 0
    assert result["comment"] == "State absent will delete IP Block with name {}".format(display_name)
    assert result["result"] is None
...
def test_absent_to_do_nothing_in_test_mode():
    ...
    assert result is not None
    assert result["changes"].__len__() == 0
    assert result["comment"] == "State absent will do nothing as no IP Block found with name {}".format(display_name)
    assert result["result"] is None
...
def test_absent_with_error_from_update():
    ...
    with patch.dict(
        ...
        result = nsxt_ip_blocks.absent(name="test_present_using_basic_auth",
                                       hostname=mocked_hostname,
                                       username="username",
                                       password="password",
                                       description="Sample description",
