from unittest.mock import create_autospec
from unittest.mock import MagicMock
from unittest.mock import patch

import pytest
import saltext.vmware.modules.vmc_org_users as vmc_org_users_exec
import saltext.vmware.states.vmc_org_users as vmc_org_users

_user_data = [{}]


_users_data = [
    _user_data[0],
    {},
]


@pytest.fixture
def mocked_ok_response():
    response = {}
    return response


@pytest.fixture
def mocked_error_response():
    error_response = {
        "error": "The credentials were incorrect or the account specified has been locked."
    }
    return error_response


@pytest.fixture
def configure_loader_modules():
    return {vmc_org_users: {}}


def test_absent_state_to_remove_user_when_module_returns_success_response(mocked_ok_response):
    mock_get_by_id_response = create_autospec(
        vmc_org_users_exec.get_by_id, return_value=mocked_ok_response
    )
    mock_delete_response = create_autospec(
        vmc_org_users_exec.delete,
        ok=True,
        return_value="user removed Successfully",
    )
    user_id = mocked_ok_response["id"]

    with patch.dict(
        vmc_org_users.__salt__,
        {
            "vmc_org_users.get_by_id": mock_get_by_id_response,
            "vmc_org_users.remove": mock_delete_response,
        },
    ):
        result = vmc_org_users.absent(
            name=user_id,
            hostname="hostname",
            refresh_key="refresh_key",
            authorization_host="authorization_host",
            org_id="org_id",
            force_delete=False,
            retain_configuration=False,
            template_name=None,
        )

    assert result is not None
    assert result["changes"] == {"new": None, "old": mocked_ok_response}
    assert result["comment"] == "Removed user {}".format(user_id)
    assert result["result"]


def test_absent_state_when_object_to_delete_does_not_exists(mocked_ok_response):
    mock_get_by_id_response = create_autospec(vmc_org_users_exec.get_by_id, return_value={})
    user_id = mocked_ok_response["id"]

    with patch.dict(
        vmc_org_users.__salt__,
        {"vmc_org_users.get_by_id": mock_get_by_id_response},
    ):
        result = vmc_org_users.absent(
            name=user_id,
            hostname="hostname",
            refresh_key="refresh_key",
            authorization_host="authorization_host",
            org_id="org_id",
            force_delete=False,
            retain_configuration=False,
            template_name=None,
        )

    assert result is not None
    assert result["changes"] == {}
    assert result[
        "comment"
    ] == "No user found with ID {} or deletion is already in progress or SDDC is still deploying".format(
        user_id
    )
    assert result["result"]


def test_absent_state_to_delete_when_opts_test_mode_is_true(mocked_ok_response):
    mock_get_by_id_response = create_autospec(
        vmc_org_users_exec.get_by_id,
        return_value={"results": [mocked_ok_response]},
    )
    user_id = mocked_ok_response["id"]

    with patch.dict(
        vmc_org_users.__salt__,
        {"vmc_sddc.get_by_id": mock_get_by_id_response},
    ):
        with patch.dict(vmc_org_users.__opts__, {"test": True}):
            result = vmc_org_users.absent(
                name=user_id,
                hostname="hostname",
                refresh_key="refresh_key",
                authorization_host="authorization_host",
                org_id="org_id",
                force_delete=False,
                retain_configuration=False,
                template_name=None,
            )

    assert result is not None
    assert len(result["changes"]) == 0
    assert result["comment"] == "State absent will delete SDDC with ID {}".format(user_id)
    assert result["result"] is None


def test_absent_state_when_object_to_delete_does_not_exists_and_opts_test_mode_is_true(
    mocked_ok_response,
):
    mock_get_by_id_response = create_autospec(vmc_org_users_exec.get_by_id, return_value={})
    user_id = mocked_ok_response["id"]

    with patch.dict(
        vmc_org_users.__salt__,
        {"vmc_sddc.get_by_id": mock_get_by_id_response},
    ):
        with patch.dict(vmc_org_users.__opts__, {"test": True}):
            result = vmc_org_users.absent(
                name=user_id,
                hostname="hostname",
                refresh_key="refresh_key",
                authorization_host="authorization_host",
                org_id="org_id",
                force_delete=False,
                retain_configuration=False,
                template_name=None,
            )

    assert result is not None
    assert len(result["changes"]) == 0
    assert result[
        "comment"
    ] == "State absent will do nothing as no SDDC found with ID {} or deletion is already in progress or SDDC is still deploying".format(
        user_id
    )
    assert result["result"] is None


def test_absent_with_error_from_delete(mocked_ok_response, mocked_error_response):
    mock_get_by_id = create_autospec(
        vmc_org_users_exec.get_by_id,
        return_value={"results": [mocked_ok_response]},
    )
    mock_delete = create_autospec(vmc_org_users_exec.delete, return_value=mocked_error_response)

    with patch.dict(
        vmc_org_users.__salt__,
        {
            "vmc_sddc.get_by_id": mock_get_by_id,
            "vmc_sddc.delete": mock_delete,
        },
    ):
        result = vmc_org_users.absent(
            name="user_id",
            hostname="hostname",
            refresh_key="refresh_key",
            authorization_host="authorization_host",
            org_id="org_id",
            force_delete=False,
            retain_configuration=False,
            template_name=None,
        )

    assert result is not None
    assert result["changes"] == {}
    assert (
        result["comment"]
        == "The credentials were incorrect or the account specified has been locked."
    )
    assert not result["result"]


def test_absent_state_when_error_from_get_by_id(mocked_ok_response, mocked_error_response):
    mock_get_by_id = create_autospec(
        vmc_org_users_exec.get_by_id, return_value=mocked_error_response
    )

    with patch.dict(
        vmc_org_users.__salt__,
        {"vmc_sddc.get_by_id": mock_get_by_id},
    ):
        result = vmc_org_users.absent(
            name="user_id",
            hostname="hostname",
            refresh_key="refresh_key",
            authorization_host="authorization_host",
            org_id="org_id",
            force_delete=False,
            retain_configuration=False,
            template_name=None,
        )

    assert result is not None
    assert result["changes"] == {}
    assert (
        result["comment"]
        == "The credentials were incorrect or the account specified has been locked."
    )
    assert not result["result"]


def test_present_state_when_error_from_getting_sddc_list(mocked_ok_response, mocked_error_response):
    mock_sddc_list = create_autospec(vmc_org_users_exec.list_, return_value=mocked_error_response)

    with patch.dict(
        vmc_org_users.__salt__,
        {"vmc_sddc.list": mock_sddc_list},
    ):
        result = vmc_org_users.present(
            name="sddc_name",
            hostname="hostname",
            refresh_key="refresh_key",
            authorization_host="authorization_host",
            org_id="org_id",
            num_hosts=1,
            provider="ZEROCLOUD",
            region="us-west-1",
        )

    assert result is not None
    assert result["changes"] == {}
    assert (
        result["comment"]
        == "Failed to get SDDCs for given org : The credentials were incorrect or the account specified has been locked."
    )
    assert not result["result"]


def test_present_state_when_error_from_create(mocked_error_response):
    mock_sddc_list = create_autospec(vmc_org_users_exec.list_, return_value={})
    mock_create = create_autospec(vmc_org_users_exec.create, return_value=mocked_error_response)

    with patch.dict(
        vmc_org_users.__salt__,
        {
            "vmc_sddc.list": mock_sddc_list,
            "vmc_sddc.create": mock_create,
        },
    ):
        result = vmc_org_users.present(
            name="sddc_name",
            hostname="hostname",
            refresh_key="refresh_key",
            authorization_host="authorization_host",
            org_id="org_id",
            num_hosts=1,
            provider="ZEROCLOUD",
            region="us-west-1",
        )

    assert result is not None
    assert result["changes"] == {}
    assert (
        result["comment"]
        == "Failed to add SDDC : The credentials were incorrect or the account specified has been locked."
    )
    assert not result["result"]


def test_present_to_create_when_module_returns_success_response(mocked_ok_response):
    mock_user_list = create_autospec(vmc_org_users_exec.list_, return_value={})
    mock_create_response = create_autospec(
        vmc_org_users_exec.create, return_value=mocked_ok_response
    )
    user_email = mocked_ok_response["id"]

    with patch.dict(
        vmc_org_users.__salt__,
        {
            "vmc_org_users.list": mock_user_list,
            "vmc_org_users.create": mock_create_response,
        },
    ):
        result = vmc_org_users.present(
            name=user_email,
            hostname="hostname",
            refresh_key="refresh_key",
            authorization_host="authorization_host",
            org_id="org_id",
            num_hosts=1,
            provider="ZEROCLOUD",
            region="us-west-1",
        )

    assert result is not None
    assert result["changes"]["new"] == mocked_ok_response
    assert result["changes"]["old"] is None
    assert result["comment"] == "Created SDDC {}".format(user_email)
    assert result["result"]


def test_present_state_for_create_when_opts_test_is_true(mocked_ok_response):
    mock_user_list = create_autospec(vmc_org_users_exec.list_, return_value={})
    user_email = mocked_ok_response["username"]

    with patch.dict(
        vmc_org_users.__salt__,
        {"vmc_org_users.list": mock_user_list},
    ):
        with patch.dict(vmc_org_users.__opts__, {"test": True}):
            result = vmc_org_users.present(
                name=user_email,
                hostname="hostname",
                refresh_key="refresh_key",
                authorization_host="authorization_host",
                org_id="org_id",
                num_hosts=1,
                provider="ZEROCLOUD",
                region="us-west-1",
            )

    assert result is not None
    assert len(result["changes"]) == 0
    assert result["comment"] == "user {} would have been created".format(user_email)
    assert result["result"] is None


@pytest.mark.parametrize(
    "actual_args",
    [
        # all actual args are None
        ({}),
        # all args have values
        (
            {
                "account_link_config": {"delay_account_link": False},
                "account_link_sddc_config": [
                    {"connected_account_id": "123", "customer_subnet_ids": ["awx:qee:asd"]}
                ],
                "validate_only": False,
            }
        ),
    ],
)
def test_present_state_during_create_should_correctly_pass_args(mocked_ok_response, actual_args):
    mock_user_list = create_autospec(vmc_org_users_exec.list_, return_value={})
    mock_create_response = mocked_ok_response.copy()

    common_actual_args = {
        "hostname": "hostname",
        "refresh_key": "refresh_key",
        "org_id": "org_id",
        "verify_ssl": False,
    }

    mock_create_response.update(actual_args)
    actual_args.update(common_actual_args)
    mock_create = create_autospec(vmc_org_users_exec.create, return_value=mock_create_response)

    with patch.dict(
        vmc_org_users.__salt__,
        {
            "vmc_org_users.list": mock_user_list,
            "vmc_org_users.create": mock_create,
        },
    ):
        result = vmc_org_users.present(name=mocked_ok_response["name"], **actual_args)

    call_kwargs = mock_create.mock_calls[0][-1]

    subset = {k: v for k, v in call_kwargs.items() if k in actual_args}
    assert subset == actual_args

    assert result is not None
    assert result["changes"]["old"] is None
    assert result["changes"]["new"] == mock_create_response
    assert result["comment"] == "Invited user {}".format(mocked_ok_response["name"])
    assert result["result"]
