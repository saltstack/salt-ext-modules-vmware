...
def test_present_test_mode():
    ...
    with patch.dict(
        ...
        with patch.dict(nsxt_license.__opts__, {"test": True}):
            ...
            assert nsxt_license.present("create-license", "hostname", "admin", "password", "license_key") == ret
...
def test_present_create_new_license():
    ...
    get_licenses_data = {
        ...
        'results': [{'capacity_type': 'USER', 'description': 'NSX Data Center Enterprise Plus', 'expiry': 0,
                     'is_eval': False, 'is_expired': False, 'license_key': 'dummy-key-license', 'quantity': 0}]
    }
    ...
    get_licenses_data_after_apply = {
        ...
        'results': [{'capacity_type': 'USER', 'description': 'NSX Data Center Enterprise Plus', 'expiry': 0,
                     'is_eval': False, 'is_expired': False, 'license_key': 'dummy-key-license', 'quantity': 0},
                    {'capacity_type': 'USER', 'description': 'NSX Data Center Enterprise Plus', 'expiry': 0,
                     'is_eval': False, 'is_expired': False, 'license_key': 'dummy-key', 'quantity': 0}
                    ]
    }
    mock_get_licenses = MagicMock(side_effect=[get_licenses_data, get_licenses_data_after_apply])
    apply_license_data = {'capacity_type': 'CPU', 'description': 'NSX Data Center Enterprise Plus', 'expiry': 0,
                          'is_eval': False, 'is_expired': False, 'license_key': 'dummy-key', 'quantity': 0}
...
def test_present_get_licenses_error():
    ...
    with patch.dict(
        ...
        ret["result"] = False
        ret["comment"] = "Failed to get current licenses from NSX-T Manager :" \
...
def test_present_apply_license_error():
    ...
    apply_license_data = {"error": "Error occurred while applying the license. Please check logs for more details."}
    mock_apply_license = MagicMock(return_value=apply_license_data)
    with patch.dict(
            nsxt_license.__salt__,
            {
        ...
        ret["result"] = False
        ret["comment"] = "Failed to apply license to NSX-T Manager :" \
...
def test_present_get_license_error_after_apply():
    ...
    with patch.dict(
        ...
        ret["result"] = False
        ret["comment"] = "Failed to retrieve licenses after applying current license from NSX-T Manager : " \
                         "Error occurred while retrieving the license. Please check logs for more details."
...
def test_absent_license_does_not_exists():
    ...
    with patch.dict(
        ...
        assert nsxt_license.absent("create-license", "hostname", "admin", "password", "different-key") == ret
...
def test_absent_apply_license_error():
    ...
    mock_get_licenses = MagicMock(side_effect=[get_licenses_data])
    delete_license_data = {"error": "Error occurred while deleting the license. Please check logs for more details."}
    mock_delete_license = MagicMock(return_value=delete_license_data)
    with patch.dict(
            nsxt_license.__salt__,
            {
        ...
        ret["result"] = False
        ret["comment"] = "Failed to delete license from NSX-T Manager :" \
...
def test_absent_get_license_error_after_delete():
    ...
    with patch.dict(
        ...
        ret["result"] = False
        ret["comment"] = "Failed to retrieve licenses after deleting current license from NSX-T Manager : " \
                         "Error occurred while retrieving the license. Please check logs for more details."

        with patch.dict(nsxt_license.__opts__, {"test": False}):
            assert nsxt_license.absent("delete-license", "hostname", "admin", "password", "dummy-key") == ret
