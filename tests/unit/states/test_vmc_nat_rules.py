...
def test_present_state_when_error_from_get_by_id(get_mocked_data):
    ...
    mock_get_by_id = create_autospec(vmc_nat_rules_exec.get_by_id, return_value=mocked_error_response)
...
def test_present_state_during_update_to_add_a_new_field(get_mocked_data):
    ...
    mock_get_by_id = create_autospec(vmc_nat_rules_exec.get_by_id, side_effect=[mocked_ok_response, mocked_updated_response])
...
def test_present_to_create_when_module_returns_success_response(get_mocked_data):
    ...
    mock_create_response = create_autospec(vmc_nat_rules_exec.create, return_value=mocked_ok_response)
    nat_rule = mocked_ok_response["id"]
...
def test_present_to_update_when_module_returns_success_response(get_mocked_data):
    ...
    mock_get_by_id_response = create_autospec(vmc_nat_rules_exec.get_by_id, side_effect=[mocked_ok_response, mocked_updated_nat_rule])
    mock_update_response = create_autospec(vmc_nat_rules_exec.update, return_value=mocked_updated_nat_rule)
    nat_rule = mocked_ok_response["id"]
...
def test_present_to_update_when_get_by_id_after_update_returns_error(get_mocked_data):
    ...
    mock_get_by_id_response = create_autospec(vmc_nat_rules_exec.get_by_id, side_effect=[mocked_ok_response, mocked_error_response])
    mock_update_response = create_autospec(vmc_nat_rules_exec.update, return_value=mocked_updated_nat_rule)
    nat_rule = mocked_ok_response["id"]
...
def test_present_to_update_when_user_input_and_existing_nat_rule_has_identical_fields(get_mocked_data):
    mocked_hostname, mocked_ok_response, mocked_error_response = get_mocked_data

    mock_get_by_id_response = create_autospec(vmc_nat_rules_exec.get_by_id, return_value=mocked_ok_response)
...
def test_absent_state_to_delete_when_module_returns_success_response(get_mocked_data):
    ...
    mock_get_by_id_response = create_autospec(vmc_nat_rules_exec.get_by_id, return_value=mocked_ok_response)
    mock_delete_response = create_autospec(vmc_nat_rules_exec.delete, ok=True, return_value="Nat rule Deleted Successfully")
    nat_rule = mocked_ok_response["id"]
...
def test_absent_state_to_delete_when_opts_test_mode_is_true(get_mocked_data):
    ...
    mock_get_by_id_response = create_autospec(vmc_nat_rules_exec.get_by_id, return_value={"results": [mocked_ok_response]})
    nat_rule = mocked_ok_response["id"]
...
def test_absent_state_when_object_to_delete_doesn_not_exists_and_opts_test_mode_is_true(get_mocked_data):
    mocked_hostname, mocked_ok_response, mocked_error_response = get_mocked_data
...
def test_absent_with_error_from_delete(get_mocked_data):
    ...
    mock_get_by_id = create_autospec(vmc_nat_rules_exec.get_by_id, return_value={"results": [mocked_ok_response]})
